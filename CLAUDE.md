...
The public institution census is a separate launch-critical contract. Read `data/public-institution-ledger.json`; never infer it from route or profile totals. Press copy must state the registry revision and preserve the canonical case-filed / multi-source corroborated / single-source attested tiers. Historical phases, leads, context, Canadian comparators, and other international institutions never enter the U.S. count.
...
- `data/registry-unified.json` is the **living canonical unified parish registry as of Registry Revision 8 (2026-07-31)**. `culturenet-brain` and CultureNet retain source research and submit adjudicated research packets by held PR; never replace this file with a flat extraction dump. Every content change must follow `docs/REGISTRY-INGESTION-CONTRACT.md`, bump `registryRevision.version`, update its changelog/hash/release counts, and pass the identity, scope, and public-source guards.
