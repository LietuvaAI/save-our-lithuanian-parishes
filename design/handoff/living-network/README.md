...
A full replacement of the existing "Lithuanian Catholic life today" page as **"The living network."** It stays at `/lithuanian-catholic-life-today`; do not create a parallel route. One page tells the current-state story: a hero map of the 14 places where Lithuanian worship still gathers (over the 9 states that hold them), a stat panel, then every community as a card — active parishes and missions, hosted Masses, the rest of the Sielovada listing, the separately documented wider Catholic-life layer, and parishes tracked outside the listing — with campaign/watch situations attached to each community's own card, never duplicated into a separate list.
...
- `wider_catholic_life` in the canonical infographic projection: two documented current religious houses (Kennebunk and Putnam) and the separately documented Atlanta occasional-worship community. These three records remain outside the 14-place regular-worship count, the 20-entry Sielovada count, the 155-institution census, and the 137 Roman Catholic parish-and-mission histories. Kennebunk is also one of the 20 Sielovada entries; render it once, in the wider-life section.
...
- The four new line drawings have landed in the rights-cleared photo registry on the handoff's base: Rochester (St. George at Our Lady of Lourdes), Washington (Epiphany), New Philadelphia (Holy Cross), and Scranton (Holy Rosary). Resolve them through the production photo-rights gate; do not preserve the prototype's empty frames for those cards.
- The Cleveland "Closed by the Diocese, Reopened by Rome" dispatch (the Polish St. Casimir story) has been removed from both the Hearth and current Brain projection. It must not render anywhere. The reference fixtures do not contain it.
- The Washington point sits at the DC shape center (828, 267.5 in the 975×610 frame) so the state hit-test shades DC, not Maryland.
- The bundled JSON files are dated design fixtures, not production inputs. They are known to predate later adjudications, including the corrected Beverly Shores planning language, the direction of the Freeland consolidation, and the final August 2 Waterbury worship outcome. Production must read the current projections and join alerts to campaigns by canonical profile identity.
...
2. **Map + stat panel** — wrapping flex row (map `flex: 1.6 1 420px`, stats `flex: 1 1 280px`, gap 30–34px), top border 1px #1c1917, so it stacks on phones.
   ...
   - **Dots** (r=7; hover r=8.5 + 1.6px #1c1917 ring): active parish = solid #2d6a4f (1px #fffdf9 stroke); mission = hollow (#fffdf9 fill, 2px #2d6a4f stroke); hosted Mass = #d5c28b fill, 1px #8a7a4e stroke. The three wider Catholic-life records remain visible with the existing distinct triangle vocabulary: solid ink for religious houses and hollow gold for the occasional Atlanta community. They do not change the 14-place figure or the nine-state shading. Native `<title>` tooltip AND a live readout line under the map (12.5px, #57534e, `aria-live=polite`): **English name first, Lithuanian in parentheses** — "St. Peter Lithuanian Parish (Bostono Šv. Petro lietuvių parapija) — Boston, MA. {ministry line}". Clicking a mark smooth-scrolls to that community's card (cards carry `id={slug}`, `scroll-margin-top: 20px`).
...
5. **Section: "Wider Lithuanian Catholic life"** — count 3. Preserve the existing canonical records and their `/catholic-life/*` pages: the Kennebunk Franciscan friary, the Putnam Immaculate Conception convent and center, and the Atlanta occasional-worship community. Explain that they are documented separately and do not change the 14-place regular-worship count. Kennebunk renders here and nowhere else on the page.
6. **Section: "Listed by Sielovada, without regular Lithuanian worship"** — count 5 after Kennebunk moves to the wider-life section. Explain that Sielovada lists 20 U.S. entries: 14 regular-worship places, Kennebunk shown in the wider-life section, and these five remaining entries — one contested closure and four with no verified regular Lithuanian Mass. Keep the stated absences as visible as the network itself.
7. **Section: "Also being tracked"** — derive the count from current alerts. These parishes are off the Sielovada listing because their Lithuanian parish life has already ended; Sielovada lists only communities with current pastoral care; the project tracks them anyway (closure appeals, planning-area consolidations, buildings on the market, or proposed demolition).
8. **Footer line** (12px #78716c, top border #e7e5e4): derive checked and revision dates from the current projections. "Every community name links to its full profile where one exists. Map positions come from the project's shared geographic layer; Washington is placed at city level."
...
- `prototype.dc.html` — the full prototype (markup + logic in one file; reference only).
