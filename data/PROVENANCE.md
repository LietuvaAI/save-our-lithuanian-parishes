...
- **Local delta (2026-07-27, issue #89 adjudication — pending upstream mirror):** documentation-field edits only, no structural changes — the Cambridge titular conflict note is marked adjudicated; `casimir-chicago-il`, `st-casimir-chicago-heights-il`, `parish-kearny-nj` and its three Kearny/Harrison siblings carry new adjudication conflict notes with citations; `parish-kearny-nj` gains name variants "Sopulingosios Dievo Motinos" / "Our Lady of Sorrows"; and two site-owned `geo` blocks move from city-centroid to the adjudicated exact church sites (`casimir-chicago-il` → 283 E 14th St, Chicago Heights; `parish-kearny-nj` → 136 Davis Ave, Kearny). Top-level `counts` metadata left untouched (its geo-precision tallies predate the site-side geocoding and are approximate). The record merges themselves are queued upstream — see `docs/CLASSIFICATION.md` standing corrections queue.
