...
- **Canonical source as of Registry Revision 4 (2026-07-28): this file in `LietuvaAI/save-our-lithuanian-parishes`.** `culturenet-brain` retains the pinned builder, final upstream snapshot, exclusions/ambiguity artifacts, and page-cited research provenance; subsequent research enters here by held PR. This is the agreed canonical-home move, not a fork.
- **Revision 2–4 evidence and adjudication:** Revision 2 applies `culturenet-brain/docs/research/elip-2026/adjudications.md`; Revision 3 applies `culturenet-brain/docs/research/parish-canon-beyond-83/tranche-1/`; Revision 4 moves the unresolved unnamed 1902 Waterbury lead to `data/candidates/waterbury-1902-unresolved-lead.json`. Together they correct Scranton/Wilkes-Barre closure identities, resolve Nanticoke/Hanover, separate Minersville's building closure from its juridic parish end, collapse Worcester, Wilkes-Barre, and DuBois duplicate records, restore Waterbury All Saints as a distinct historical entity, mark Providence as a historical phase, separate Roman Catholic DuBois St. Joseph from the provisional 1903 national attempt, and keep the protected Waterbury St. Joseph campaign free of unresolved historical leads. Replayable applications and human-readable reports live in `scripts/apply-registry-revision*.mjs` and `data/candidates/registry-revision-*-report.md`.
...
- **Counts:** 210 canonical records after Registry Revision 4: 195 `parish`, 1 historical `phase`, 4 `misija`, and 10 `congregation` records. The 83 historical U.S. case-file rows remain explicitly accounted for across 82 unique case-filed entities (`c83_rows`), because the false Waterbury St. Casimir row remains merged into St. Joseph. Worcester, Wilkes-Barre, and DuBois each lost one duplicate shell; Providence remains visible as phase evidence without inflating the durable-parish count; the unnamed 1902 Waterbury lead is preserved outside public counts pending identity adjudication.
...
- **Mutation guard:** `registryRevision.version`, dated changelog, and SHA-256 content hash are mandatory. `scripts/verify-registry-revision.mjs` runs first in `npm run data` and fails if content changes without a revision bump.
- **Campaign identity guard:** every public campaign in `data/alerts.json` carries an `identityLock`. `scripts/verify-campaign-identity.mjs` blocks the data build if its canonical profile, registry row, Lithuanian name, location, institution type, ownership, founding-year anchor, denomination, or C83 lineage changes. Current status remains updateable as documented events unfold.
